...
GestureKey := "XButton1"
GestureKeyClean := "XButton1"
;MsgBox, %GestureKey%
hotkey, %GestureKey%, Recognize
hotkey, *LButton, act1
hotkey, *RButton, act2
hotkey, *MButton, act3
hotkey, *WheelDown, actDown
hotkey, *WheelUp, actUp
hotkey, *LButton, Off
hotkey, *MButton, Off
hotkey, *RButton, Off
...
act2:
	...
	If ButtonState = D
	{
		/*
		MouseGetPos,,,KDE_id
		WinGet, KDE_clid, ahk_id %KDE_id%
		If KDE_clid = ahk_class ConsoleWindowClass
		   ControlSend, ahk_parent, ^{space}c, ahk_id %KDE_id%
		else
...
ButtonUp:
	...
	{
		mg_ctrl := 0
		Send, {ctrl up} ;! maybe set to what it was prior
		;SoundPlay, *48
	}
return
...
SimulateClick:
	;SoundPlay, *64
    WinGetClass, CurClass, A
    ;MsgBox, %CurClass%
    if CurClass = TfrmWOTMain
    {
        send, ^!{left}
    }
    else
    {
		;MouseClick, X1, %MousePosX0%, %MousePosY0%, 1, 0, D
		;MouseClick, X1, %MousePosX%, %MousePosY%, 1, 0, U
		MouseClick, X1
		MousePosNumber = 0
		WindowID = 0
    }
...
ButtonHold:
  ...
  ; Detect if the mouse button is being held or being released
  ;-------------------------------------------------------------
  GetKeyState, MouseButtonState2, %GestureKeyClean%, P
  ...
  MouseRectSquareSize := MouseRectWidth + MouseRectHeight
  If MouseRectSquareSize < 30
  {
	;MsgBox, %MouseRectSquareSize% : %MousePosXMax%,%MousePosXMin%
 	gosub SimulateClick
	Return
  }
